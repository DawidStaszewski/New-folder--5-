<template>
    <div class="p-8 bg-gray-900 text-white min-h-screen">
        ...
        <div class="max-w-3xl mx-auto bg-gray-800 p-6 rounded-lg shadow-lg">
            <form @submit.prevent="addOffer">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <div v-for="type in offerTypes" :key="type.id" class="flex items-center">
                            ...
                            <label :for="'offer_type_' + type.id"
                                :class="['py-2 px-4 rounded-lg cursor-pointer transition duration-200', offer.type === type.id ? 'bg-green-900 text-white' : 'bg-gray-700 text-gray-300']">
                                {{ type.name }}
                            </label>
                        </div>
                    </div>
                ...
                <!-- Wybór kompetencji -->
                <div class="mb-6">
                    <label for="offer_competence" class="block text-sm font-medium text-gray-300 mb-4">Kompetencje:</label>
                    <div class="flex flex-wrap gap-4">
                        <div v-for="competence in offerCompetences" :key="competence.id" class="flex items-center">
                            <!-- Zmieniamy na checkbox, aby można było zaznaczyć więcej niż jedną kompetencję -->
                            <input type="checkbox" :id="'offer_competence_' + competence.id" v-model="offer.competences"
                                :value="competence.id" class="hidden" />
                            <label :for="'offer_competence_' + competence.id"
                                :class="['py-2 px-4 rounded-lg cursor-pointer transition duration-200', offer.competences.includes(competence.id) ? 'bg-blue-500 text-white' : 'bg-gray-700 text-gray-300']">
                                {{ competence.name }}
                            </label>
                        </div>
                    </div>
  ...
  const offer = ref({
    ...
    expiration_date: "",
    type: "",
    competences: [], // Zmieniamy to na tablicę dla wielu kompetencji
  });
  
  const offerTypes = ref([]);
  const offerCompetences = ref([]);
  ...
  const fetchOfferCompetences = async () => {
    try {
        const response = await api.getOfferCompetences();
        const data = response.data;
  
        if (Array.isArray(data)) {
            offerCompetences.value = data;
        } else {
            errorMessage.value = "Błąd: Odpowiedź API nie zawiera tablicy.";
        }
    } catch (error) {
        console.error("Error fetching offer competences:", error);
        errorMessage.value = "Nie udało się pobrać danych użytkownika.";
    }
  };
  
  onMounted(fetchOfferTypes);
  onMounted(fetchOfferCompetences);
  ...
  const addOffer = async () => {
    try {
        ...
        errorMessage.value = "";
        offer.value = { title: "", description: "", expiration_date: "", type: "", competences: [] }; // Zresetowanie wartości
    } catch (err) {
